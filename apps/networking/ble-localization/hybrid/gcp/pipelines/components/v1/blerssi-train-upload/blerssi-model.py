...
def main(unused_args):
    ...
    for dir in os.listdir(FLAGS.tf_model_dir):
        if re.match('[0-9]',dir):
            version_path=os.path.join(FLAGS.tf_model_dir,dir)
            break
    ...
    for f in os.listdir(version_path):
        if os.path.isdir(os.path.join(version_path, f)):
            for f1 in os.listdir(os.path.join(version_path, f)):
                destination_blob_name=f1
                blob = bucket.blob("blerssi/%s/%s/"%(FLAGS.model_version,f)+destination_blob_name)
                blob.upload_from_filename(os.path.join(version_path, f, f1))
        else:
            destination_blob_name=f
            blob = bucket.blob("blerssi/%s/"%FLAGS.model_version+destination_blob_name)
            blob.upload_from_filename(os.path.join(version_path, destination_blob_name))
